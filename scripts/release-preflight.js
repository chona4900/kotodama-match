...
requireText('index.html', 'state-migrations.js');
forbidText('index.html', 'id="infoNavigation"');
requireText('index.html', "chooseBattleAction('attack')");
requireText('main.js', 'function chooseBattleAction(action)');
requireText('main.js', 'function getDisplayedStats()');
requireText('main.js', 'didResetKnownTestState');
...
forbidText('main.js', 'function preventViewportZoom()');
...
forbidText('index.html', 'user-scalable=no');
forbidText('index.html', 'maximum-scale=');
...
requireText('style.css', '@media (orientation: landscape) and (max-height: 500px)');
...
requireText('index.html', 'id="kotodamaCupMenu"');
requireText('index.html', 'id="postMatchStamps"');
requireText('index.html', 'id="onlineProfileConsent"');
forbidText('index.html', 'fonts.googleapis.com');
requireText('main.js', 'function openKotodamaCupMenu(');
requireText('main.js', 'function sendPostMatchStamp(stamp)');
requireText('main.js', 'function deleteOnlineProfileForReset(');
requireText('main.js', 'ONLINE_PROFILE_CONSENT_STORAGE_KEY');
requireText('main.js', 'ACTIVE_AWARD_STORAGE_KEY');
requireText('online-battle/src/index.mjs', 'recordRankedMatch');
requireText('online-battle/src/index.mjs', 'if (playerId) await authenticateProfile');
requireText('online-battle/src/index.mjs', 'includeProfile: false');
requireText('online-battle/src/ranking-rules.mjs', 'export const DAILY_WIN_LIMIT = 3');
requireText('online-battle/src/ranking-store.mjs', 'getWeeklyRankings');
requireText('online-battle/src/ranking-store.mjs', 'isMe: Boolean(isMe)');
requireText('online-battle/migrations/0001_kotodama_cup.sql', 'CREATE TABLE IF NOT EXISTS profiles');
requireText('online-battle/migrations/0001_kotodama_cup.sql', 'CREATE TABLE IF NOT EXISTS daily_win_caps');
requireText('online-battle/wrangler.jsonc', '"binding": "RANKINGS_DB"');
requireText('privacy.html', 'オンライン対戦とコトダマ杯');
requireText('docs/ranking-release-checklist.md', 'Google Play Console');
