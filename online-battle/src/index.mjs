...
const SUPPORT_PAGE = legalPage('コトダマっち サポート', `
  ...
  <h2>データを最初からやり直す</h2>
  <p>ゲーム画面の「B リセット」を選ぶと、端末内の育成データをタマゴの状態へ戻せます。オンライン対戦を利用している場合は、匿名プロフィール、コトダマ杯の週間戦績、対戦記録、入賞・受賞履歴もサーバーから削除します。</p>
  <p>サーバー側の削除には通信が必要です。通信エラーが表示された場合は、通信できる状態でもう一度「B リセット」を行ってください。アプリを削除しただけでは、サーバー側の情報は削除されません。</p>
...
const PRIVACY_PAGE = legalPage('コトダマっち プライバシーポリシー', `
  ...
  <h2>データの削除</h2>
  <p>オンライン対戦とランキングのためのデータ保存への同意は、いつでもアプリ内の「B リセット」で取り消せます。「B リセット」を選ぶと、端末内の育成データ、戦績、解放状況を削除します。オンライン対戦を利用している場合は、同時にサーバーへ削除を要求し、匿名プロフィール、週間戦績、対戦記録、入賞・受賞履歴も削除します。</p>
  <p>通信できない状態や通信エラーのときは、サーバー側の削除が完了しないことがあります。通信できる状態で、もう一度「B リセット」を行ってください。アプリを削除すると端末内のアプリデータは削除されますが、サーバーへは通知できないため、サーバー側の情報は自動では削除されません。再操作できない場合や、削除できたか確認したい場合は、下記のお問い合わせ先までご連絡ください。</p>
  <h2>オンライン対戦とコトダマ杯</h2>
  ...
  <p>通信できないときにも前回の結果を確認できるよう、最後に読み込んだランキングを端末内に一時保存します。この情報は「B リセット」またはアプリの削除で端末から削除されます。</p>
