...
const SUPPORT_PAGE = legalPage('コトダマっち サポート', `
  ...
  <h2>データを最初からやり直す</h2>
  <p>ゲーム画面の「B リセット」を選ぶと、キャラクターだけをタマゴの状態へ戻せます。言霊の累計、戦歴、魂のおやつ、図鑑、解放状況、コトダマ杯の記録は残ります。</p>
  <p>コトダマ杯の匿名プロフィール、週間戦績、対戦記録、入賞・受賞履歴を削除したいときは、コトダマ杯の画面にある「ランキングの記録を削除」を選んでください。サーバー側の削除には通信が必要です。通信エラーが表示された場合は、通信できる状態でもう一度この操作を行ってください。アプリを削除しただけでは、サーバー側の情報は削除されません。</p>
...
const PRIVACY_PAGE = legalPage('コトダマっち プライバシーポリシー', `
  ...
  <p class="updated">最終更新日: 2026年8月25日</p>
  <h2>収集する情報</h2>
  <p>コトダマっちは、育成の進行状況、発話した言霊の回数、戦績などのゲームデータを端末内に保存します。通常プレイでは、開発者が運営するサーバーへ、これらのゲームデータや音声を送信・保存することはありません。オンライン対戦や「コトダマ杯」を利用する場合だけの通信内容は、下記「オンライン対戦とコトダマ杯」をご確認ください。</p>
  <h2>マイクと音声認識</h2>
  ...
  <h2>データの削除</h2>
  <p>ゲーム画面の「B リセット」は、キャラクターをタマゴの状態へ戻し、その回の進化回数だけをリセットします。言霊の累計、戦歴、魂のおやつ、図鑑、解放状況、コトダマ杯の記録は残ります。</p>
  <p>オンライン対戦とランキングのためのデータ保存への同意は、コトダマ杯の画面にある「ランキングの記録を削除」からいつでも取り消せます。この操作では、サーバーへ削除を要求し、匿名プロフィール、週間戦績、対戦記録、入賞・受賞履歴と端末内のランキング表示用キャッシュを削除します。通信できない状態や通信エラーのときは、サーバー側の削除が完了しないことがあります。通信できる状態で、もう一度この操作を行ってください。アプリを削除すると端末内のアプリデータは削除されますが、サーバーへは通知できないため、サーバー側の情報は自動では削除されません。再操作できない場合や、削除できたか確認したい場合は、下記のお問い合わせ先までご連絡ください。</p>
  <h2>オンライン対戦とコトダマ杯</h2>
  ...
  <p>通信できないときにも前回の結果を確認できるよう、最後に読み込んだランキングを端末内に一時保存します。この情報は「ランキングの記録を削除」またはアプリの削除で端末から削除されます。</p>
  <p>音声、音声認識結果、マイク録音、利用者が入力した名前・文章・画像、Apple AccountまたはGoogleアカウントの情報、メールアドレス、連絡先、位置情報は、オンライン対戦やランキングのために送信・保存しません。</p>
  <h2>お問い合わせ</h2>
